module.exports = {
  ...
  findOne: async (req, res) => {
    try {
      const { id } = req.params;
      const item = await OrderHistory.findOne({
        where:{
          id
        },
        include: []
  ...
  create: async (req, res) => {
    try {
      let data = req.body;
      const item = await OrderHistory.create(data);
  ...
  update: async (req, res) => {
    try {
      ...
      const item = await OrderHistory.update(data ,{
        where: { id, },
      });
      res.ok({ message, data: { item } });
    } catch (e) {
  ...
  destroy: async (req, res) => {
    try {
      const { id } = req.params;
      const item = await OrderHistory.destroy({ where: { id } });
